...
pub fn get_pagemap(virt_addr: u64, length: u64) -> Vec<i64> {
    ...
    let virt_addr_end = virt_addr + length;
    let mut pfns: Vec<i64> = Vec::new();
    ...
    while virt_addr < virt_addr_end {
        ...
        // println!("Addr: {:#20x} - PFN: {:#20x}", virt_addr, pfn);
        virt_addr += page_size;
    }
